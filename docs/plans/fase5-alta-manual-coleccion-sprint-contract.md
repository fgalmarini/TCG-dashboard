...
commiteado), Fase 4 (`docs/plans/fase4-importer-sprint-contract.md`, cerrada) y Fase 5 parcial
(`docs/plans/fase5-scryfall-magic-backfill-sprint-contract.md`, cerrada — backfill de catálogo
Magic vía Scryfall). Especifica el "próximo paso (Fase 5, trabajo principal)" descrito
en `docs/CURRENT_STATE.md`: alta manual de colección + UI de confirmación por lote para
...
- Si no matchea (carta de Magic fuera del scope LOTR, ej. reprint en el pool `5489`/
  `6665` mencionado en `docs/plans/fase2-cardmarket-hallazgos-y-schema.md` §2): alta manual —
...
- Sin match en absoluto → alta manual y reportarlo (no debería pasar, todo el catálogo
  de One Piece está importado — pero reportar, no asumir, `docs/ARCHITECTURE.md`).
...
2. Facundo completa `chosen_card_id` a mano por fila (puede abrir el link de Cardmarket
   si tiene dudas, como ya preveía `docs/plans/fase2-cardmarket-hallazgos-y-schema.md` §1).
...
copias de la misma carta, `docs/ARCHITECTURE.md`). Para no duplicar filas si Facundo vuelve a
...
- Documentar esta elección en el docstring del módulo (mismo criterio que
  `docs/plans/fase5-scryfall-magic-backfill-sprint-contract.md` §1, `AGENTS.md`).
...
## 5. Reporte final (nunca silencioso, `docs/ARCHITECTURE.md`)
...
- Dashboard/UI web (Fase 6, `docs/PRODUCT.md`) — todo este sprint es CLI + CSV, sin
  frontend nuevo.
...
- Actualizar `docs/CURRENT_STATE.md` a Fase 6 al cerrar este sprint.
...
**Re-corrida de `docs/plans/fase4-importer-sprint-contract.md` §6 (poblar `market_price_history`,
