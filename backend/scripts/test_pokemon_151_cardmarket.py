...
class Pokemon151CardmarketResolutionTest(unittest.TestCase):
    ...
    def test_dry_run_is_evidence_driven_and_does_not_write(self):
        ...
        result = resolver.run(self.db, PRODUCTS, PRICES, DISCOVERY, self.root / "reports", self.root / "backups", False)
    ...
    def test_apply_persists_canonical_scope_without_forcing_physical_finish_or_price(self):
        result = resolver.run(self.db, PRODUCTS, PRICES, DISCOVERY, self.root / "reports", self.root / "backups", True)
        ...
        second = resolver.run(self.db, PRODUCTS, PRICES, DISCOVERY, self.root / "reports2", self.root / "backups2", False)
    ...
    def test_ambiguous_same_name_products_remain_unpersisted(self):
        resolver.run(self.db, PRODUCTS, PRICES, DISCOVERY, self.root / "reports", self.root / "backups", True)
        conn = sqlite3.connect(self.db)
