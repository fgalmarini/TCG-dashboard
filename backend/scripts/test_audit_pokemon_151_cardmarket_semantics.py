...
class Pokemon151CardmarketSemanticsTest(unittest.TestCase):
    ...
    def test_apply_requires_offline(self):
        with self.assertRaises(audit.SemanticsGateError):
            audit.run(self.db, PRODUCTS, PRICES, MAPPING_DIR, self.root / "reports", CACHE, False, False, True)

    def test_offline_apply_uses_cached_evidence_and_changes_only_metric_relation(self):
        before = hashlib.sha256(self.db.read_bytes()).hexdigest()
        result = audit.run(self.db, PRODUCTS, PRICES, MAPPING_DIR, self.root / "reports", CACHE, True, False, True)
    ...
    def test_dry_run_reports_410_and_403_without_promoting(self):
        result = audit.run(self.db, PRODUCTS, PRICES, MAPPING_DIR, self.root / "reports", CACHE, True, False, False)
    ...
    def test_metric_mapping_status_supports_supported(self):
        audit.run(self.db, PRODUCTS, PRICES, MAPPING_DIR, self.root / "reports", CACHE, True, False, True)
        conn = sqlite3.connect(self.db)
