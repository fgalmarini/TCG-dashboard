...
colección), CERRADA — ver `claude/fase5-alta-manual-coleccion-sprint-contract.md`
sección 9 (98/98 filas, 0 pendientes). Especifica el "Phase 6: Build basic dashboard"
del roadmap de `AGENTS.md` §22. Decisiones de alcance confirmadas con Facundo el
...
- One Piece y Pokémon: 0 filas en `collection_items` (colección real es 100% Magic por
  ahora — ver `AGENTS.md` §23 y decisión previa de esta sesión, no es un bug).

## 1. Alcance de esta fase (confirmado con Facundo)

Del roadmap completo de `AGENTS.md` §15, esta fase implementa **solo**:
...
ejemplo de `AGENTS.md` §7 y la que Cardmarket trata como estimación estándar de valor
...
posible** — no inventar un precio ni asignarles 0 silenciosamente (`AGENTS.md` §20,
§25). Tratamiento:
...
- `GET /api/collection/{id}`
  - Detalle completo de una fila: todos los campos de `collection_items` +
    ...
    (`AGENTS.md` §25: mostrar siempre fuente + métrica + fecha).

Levantar en `http://localhost:8000` (`AGENTS.md` §17).

## 4. Frontend — `frontend/`

Vite + React + TypeScript + Tailwind CSS + shadcn/ui + Recharts (`AGENTS.md` §16).
...
mínimo (`AGENTS.md` §24, evitar overengineering) — si en algún momento hace falta
...
Levantar en `http://localhost:3000` (`AGENTS.md` §17). Dark mode y diseño responsive
desde el arranque (`AGENTS.md` §26) — no como retrofit después.

## 5. Principios a respetar (ya definidos en AGENTS.md, no reabrir)
...
1. `backend/api` levanta en `:8000`, `frontend` en `:3000`, ambos con un solo comando
   documentado en `README.md` (`AGENTS.md` §17).
...
6. Correr el proyecto localmente y confirmar dark mode + layout responsive básico
   (`AGENTS.md` §26) antes de dar la fase por cerrada.

## 7. Cierre

- Actualizar `AGENTS.md` §23 "Current Phase" a Fase 7 al cerrar este sprint.
...
*(Pendiente — completar cuando el developer cierre el sprint.)*
