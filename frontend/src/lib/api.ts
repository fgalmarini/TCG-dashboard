...
export function fetchWishlist(): Promise<WishlistListResponse> {
  return request<WishlistListResponse>('/api/wishlist')
}
...
export function updateWishlist(id: number, payload: { priority?: string; quantity_wanted?: number; max_price?: number | null; currency?: string | null; notes?: string | null }): Promise<WishlistItem> {
...
export function moveWishlistToCollection(id: number): Promise<WishlistItem> {
  return mutate<WishlistItem>(`/api/wishlist/${id}/move-to-collection`, 'POST')
}
