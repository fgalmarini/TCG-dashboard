...
(`fase4-importer-sprint-contract.md`, cerrada — no se re-abre). Origina en
`Revisión-fases 2-4 — Catálogo externo .md`, que reemplaza para Magic el mecanismo de
self-healing de `fase2-cardmarket-hallazgos-y-schema.md` sección 6. Decisiones
...
un límite duro, pero Scryfall lo pide). Cumple AGENTS.md §21 — no es scraper ni
...
de código. Documentar esta elección en el docstring del módulo (AGENTS.md §24).
...
5. **Reporte final**, mismo patrón que `backend/importer/report.py` (dataclass +
   `print_summary()`, nunca silencioso — AGENTS.md §20):
